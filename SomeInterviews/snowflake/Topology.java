...
public class Topology {
    ...
    public List<List<String>> getPrivileges(List<List<String>> privileges,int[][] grant){
        ...
        for (int i=0;i<privileges.size();i++){//开始是遍历grant的，那不对，因为有些int可能不出现grant里
        ...
        for (int i=0;i<privileges.size();i++){
            ...
            privs.get(i).addAll(privileges.get(i));
        }
        ...
        for (int i=0;i< privileges.size();i++){
            rs.add(new ArrayList<>(privs.get(i)));
        }
        return rs;
    }
}
