...
public class MaximumPerformanceofaTeam {
    ...
    public int maxPerformance2(int n, int[] speed, int[] efficiency, int k) {
        Pair[] team=new Pair[speed.length];
        for (int i=0;i<efficiency.length;i++){
            Pair p=new Pair(speed[i],efficiency[i]);
            team[i]=p;
        }
        Arrays.sort(team,(a,b)->b.efficiency-a.efficiency);
        PriorityQueue<Pair> pq=new PriorityQueue<>((a,b)->a.speed-b.speed);
        long totalSpeed=0;
        long max=0;
        for (int i=0;i<team.length;i++){
             if (pq.size()==k){
                 totalSpeed-=pq.poll().speed;
             }
             pq.offer(team[i]);
             totalSpeed+=team[i].speed;
             long eff=team[i].efficiency;
             long temp=totalSpeed*eff;
